...
export default function CustomizerView() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 fade-in-up">
      <input
        type="file"
        ref={fileInputRef}
        className="hidden"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col items-center">
          ...
          <div className="w-full bg-slate-900/5 border border-slate-200/80 rounded-3xl p-6 relative flex items-center justify-center min-h-[380px] sm:min-h-[480px]">
            <div className="w-full max-w-[420px]">
              <ClockPreview onSelectPhoto={triggerFileSelect} />
            </div>
          </div>
        </div>

        {/* Right Column: Customizer Sidebar Options Panel */}
        <div className="lg:col-span-6 bg-white rounded-3xl border border-slate-200/60 shadow-xl p-6 sm:p-8 flex flex-col gap-6 max-h-[85vh] overflow-y-auto">
          
          <div>
            <h2 className="font-heading font-black text-2xl text-slate-900 leading-tight">
              Clock Customizer Studio
            </h2>
            <p className="text-xs text-slate-500 mt-1">Design and customize your premium ticking wall clock.</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {SHAPES.map((sh) => (
                <button
                  ...
                  className={`py-2 px-2.5 border rounded-xl text-xs font-bold text-center transition-all cursor-pointer truncate ${
                    shape === sh.id
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs"
              >
              ...
              {image && (
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-600 p-2.5 rounded-xl border border-slate-200/50 cursor-pointer"
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {SIZE_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`py-2.5 px-2 border rounded-xl text-xs font-bold text-center transition-all cursor-pointer ${
                    size === opt.id
                      ? "border-blue-600 bg-blue-50/20 text-blue-700 ring-2 ring-blue-500/5"
                      : "border-slate-200 hover:border-slate-300 bg-white text-slate-600"
                  }`}
          ...
          <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-200/50 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5 border-b border-slate-200/60 pb-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                <span className="block font-bold text-slate-600 mb-1.5">Dial Style</span>
                <select
                  value={dialType}
                  onChange={(e) => updateSession({ dialType: e.target.value })}
                  className="w-full font-semibold border border-slate-200 rounded-lg p-2 bg-white outline-none"
                >
              ...
              {dialType !== "none" && dialType !== "ticks" && (
                <div>
                  <span className="block font-bold text-slate-600 mb-1.5">Numbers Font</span>
                  <select
                    value={numberFont}
            ...
            {dialType !== "none" && (
              <div>
                <span className="block text-xs font-bold text-slate-600 mb-1.5">Markings Color</span>
                <div className="flex gap-2.5 flex-wrap">
                  {PRESET_COLORS.map((col) => (
                    <button
                      ...
                      className={`w-6.5 h-6.5 rounded-full border transition-transform hover:scale-105 cursor-pointer ${
          ...
          <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-200/50 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                <span className="block font-bold text-slate-600 mb-1.5">Hands Color</span>
              ...
              <div>
                <span className="block font-bold text-slate-600 mb-1.5">Hands Movement</span>
                <select
                  value={handMovement}
            ...
            <div className="space-y-3.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full font-semibold border border-slate-200 rounded-lg p-2.5 bg-white outline-none focus:border-blue-500 focus:bg-white"
                />
              </div>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-3 px-4 rounded-xl text-xs transition-all cursor-pointer border border-slate-200/60"
            >
              Reset All Settings
            </button>
          </div>
