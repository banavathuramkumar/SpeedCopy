...
export default function ClockPreview({ onSelectPhoto, onRemovePhoto, customSession }) {
  ...
  // Customizer handles are visible only if it is the primary customizer session
  ...
  const getNumberCoords = (index, radiusOverride) => {
    ...
    if (shape === "heart") {
      if (index === 0) { // 12
        y += 18;
      } else if (index === 1 || index === 11) { // 1 or 11
        y += 12;
      } else if (index === 5 || index === 7) { // 5 or 7
        y -= 10;
      } else if (index === 6) { // 6
        y -= 25;
      }
  ...
  const handleGlobalDragMove = (e) => {
    ...
    if (dragStart.current.type === "move") {
      ...
      dispatch(updateCustomizerSession({
      ...
      dispatch(updateCustomizerSession({
        zoom: Math.min(15.0, Math.max(0.05, dragStart.current.initialZoom * ratio))
      }));
  ...
  return (
    <div className="relative aspect-square w-full flex items-center justify-center p-4 select-none">
      <div className={`relative w-full h-full transition-transform duration-500 ease-out transform ${getVisualScale()}`}>
        {/* SVG Canvas */}
        <svg
          ref={svgRef}
          viewBox="0 0 400 400"
          className={`w-full h-full clock-shadow select-none ${
            !image && isCustomizer ? "cursor-pointer" : ""
          }`}
          onClick={!image && isCustomizer ? onSelectPhoto : undefined}
          onMouseMove={handleGlobalDragMove}
          onMouseUp={handleGlobalDragEnd}
          onMouseLeave={handleGlobalDragEnd}
          onTouchMove={handleGlobalDragMove}
        ...
        >
          ...
          {/* Wall Shadows */}
          <use href={`#shape-${shape}`} className="fill-slate-950/25 blur-md" transform="translate(4, 18)" pointerEvents="none" />
          <use href={`#shape-${shape}`} className="fill-slate-950/20 blur-xs" transform="translate(2, 8)" pointerEvents="none" />

          {/* Clock Frame */}
          <use href={`#shape-${shape}`} className="fill-slate-100" pointerEvents="none" />
          ...
          <use href={`#shape-${shape}`} className="fill-none stroke-slate-200/80 stroke-2.5" pointerEvents="none" />
